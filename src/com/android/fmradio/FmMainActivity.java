...
import com.android.fmradio.views.HorizontalScaleView;
import android.support.v4.app.ActivityCompat;
import android.support.v4.content.ContextCompat;
//import android.support.v7.widget.RecylerView;
...
public class FmMainActivity extends Activity implements FmFavoriteEditDialog.EditFavoriteListener {
    ...
    //long press flag,make sure seekStation run one time when user hold the media button
    private boolean longPressFlag = false;

    // UI views
//    private TextView mTextStationName = null;pm

    private TextView mTextStationValue = null;

    // RDS text view
//    private TextView mTextRds = null;pm

    private TextView mActionBarTitle = null;

    private TextView mNoEarPhoneTxt = null;

    private ImageView mButtonPrevStation = null;

    private ImageView mButtonNextStation = null;
...
//    private FmScroller mScroller;
    ...
    private RelativeLayout mScaleLayout=null;
    /**
     *  刻度尺-横向
     */
    ...
    /*private FmScroller.EventListener  mEventListener = new EventListener() {
        @Override
...
//                        mTextStationName.setText(FmStation.getStationName(mContext, frequency));
                ...
                mButtonAddToFavorite.setImageResource(R.drawable.icon_btn_favorite_off);
                // Notify scroller
//                    mScroller.onRemoveFavorite();
            ...
            switch (v.getId()) {
                ...
                case R.id.play_button:
                    if (mService.getPowerStatus() == FmService.POWER_UP) {
                        powerDownFm();
                        mScaleView.setOnScaleChangeListener(null);
                    } else {
                        powerUpFm();
                        mScaleView.setOnScaleChangeListener(mScaleValueListener);
                    }
                    break;
                default:
    ...
    //slide Scale Listener
    private HorizontalScaleView.OnScaleValueListener mScaleValueListener=new HorizontalScaleView.OnScaleValueListener() {
        @Override
...
//                    mTextRds.setText(rtString);
    ...
    private class NoHeadsetAlpaOutListener implements AnimationListener {
        ...
        public void onAnimationEnd(Animation animation) {
            ...
            changeToMainLayout();
//            cancelMainAnimation();
            Animation anim = AnimationUtils.loadAnimation(mContext, R.anim.floatbtn_alpha_in);
//            mBtnPlayContainer.startAnimation(anim);
        ...
        public void onAnimationStart(Animation animation) {
            mNoEarphoneTextLayout.setElevation(0);
        }
    }
...
//            changeToNoHeadsetLayout();
        ...
        @Override
        public void onAnimationStart(Animation animation) {
            mNoEarphoneTextLayout.setElevation(mMiddleShadowSize);
        }
...
//            mTextStationName.setText(FmStation.getStationName(mContext, mCurrentStation));
            ...
            mButtonAddToFavorite.setImageResource(R.drawable.icon_btn_favorite_on);
            // Notify scroller
//            mScroller.onAddFavorite();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mPreferences = getSharedPreferences("com.android.fmradio_preferences", MODE_PRIVATE);
        int defaultStation = mPreferences.getInt("curent_station", 0);
        if(defaultStation>=875){
            mScaleView.setScaleDefault(defaultStation);
        }else {
            mScaleView.setScaleDefault(FmUtils.DEFAULT_STATION);
        }
...
//        mScroller.notifyAdatperChange();
    ...
    private void refreshStationUI(int station) {
        ...
        if(mScaleView!=null){
            mScaleView.setOnScaleChangeListener(null);
            mScaleView.setScaleSelectValue(mCurrentStation);
            mScaleView.setOnScaleChangeListener(mScaleValueListener);
        }
        // TODO it's on UI thread, change to sub thread
...
//        mTextRds.setText(radioText);
    ...
    public void onStart() {
        super.onStart();
       Log.d(TAG,"onStart:"+"isAntennaAvailable="+isAntennaAvailable());
        requestPermission();
    ...
    public void onResume() {
        super.onResume();
        Log.d(TAG, "onResume:Volume=" + mAudioManager.getStreamVolume(AudioManager.STREAM_MUSIC));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.fm_record_list:
                Intent intent = new Intent(FmMainActivity.this, FmRecordListActivity.class);
...
//        mTextStationName.setText(FmStation.getStationName(mContext, mCurrentStation));
    ...
    private void requestPermission() {
        ...
        if (locationPermission != PackageManager.PERMISSION_GRANTED && phonePermission != PackageManager.PERMISSION_GRANTED) {
        ...
        if (requestCode == 11) {
            for (int i = 0; i < grantResults.length; i++) {
                if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(FmMainActivity.this, "Failed to get permission", Toast.LENGTH_SHORT).show();
                }
            }
        }
    }
    ...
    private void showProgressDialog(){
        mAlertDialog = new AlertDialog.Builder(FmMainActivity.this).
    ...
    private void refreshImageButton(boolean enabled) {
        boolean isPowerUp = (mService.getPowerStatus() == FmService.POWER_UP);
        mButtonNextStation.setImageResource((isPowerUp
                ? R.drawable.radio_btn_next_rest
                : R.drawable.radio_btn_next_disable));

        mButtonPrevStation.setImageResource((isPowerUp
                ? R.drawable.radio_btn_previewa_rest
                : R.drawable.radio_btn_previewa_disable));

        mButtonPrevStation.setEnabled(enabled);
        mButtonNextStation.setEnabled(enabled);
        mButtonAddToFavorite.setEnabled(enabled);

        mScaleView.setEnabled(enabled);
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE:
                return true;
            /** lusb1
             *  listen for the click event of the headset
             *  start
             **/
            case KeyEvent.KEYCODE_HEADSETHOOK:
                if(event.getRepeatCount()==0){
                    if (mService.getPowerStatus() == FmService.POWER_UP&&mIsPower) {
                        powerDownFm();
                    } else if(mService.getPowerStatus()==FmService.POWER_DOWN&&!mIsPower){
                        powerUpFm();
                    }
                    return true;
                } else {
                    if (!longPressFlag) {
                        ...
                        intent.putExtra(RecognizerIntent.EXTRA_PROMPT, "anyway");
                        powerDownFm();
...
//        mTextRds = (TextView) findViewById(R.id.station_rds);
        ...
        mButtonAddToFavorite = (ImageView) findViewById(R.id.button_add_to_favorite);
//        mTextStationName = (TextView) findViewById(R.id.station_name);
        mButtonPrevStation = (ImageView) findViewById(R.id.button_prevstation);
        mButtonNextStation = (ImageView) findViewById(R.id.button_nextstation);
        mScaleLayout = (RelativeLayout) findViewById(R.id.rl_scale_container);
        ...
        // l new
        mMainLayout = (RelativeLayout) findViewById(R.id.main_view);
        ...
        mButtonPlay = (ImageView) findViewById(R.id.play_button);
        ...
        //Scale
        mScaleView = new HorizontalScaleView(this);
        mScaleView.setId(R.id.scale_view);
        mScaleView.setScaleRange(875, 1080);
        mScaleView.setDirection(HorizontalScaleView.DIRECTION_BOTTOM);
        mScaleView.setScaleWidth(42);
        mScaleView.setHorizontalScrollBarEnabled(false);
        mScaleLayout.addView(mScaleView, 1440, 200);
        ((RelativeLayout.LayoutParams) mScaleView.getLayoutParams()).addRule(RelativeLayout.CENTER_IN_PARENT);

        mIndexView = new View(this);
        mIndexView.setId(R.id.index_view);
        mIndexView.setBackgroundColor(Color.RED);
        mIndexView.setBackgroundResource(R.drawable.radio_calibration_pointer);
        mScaleLayout.addView(mIndexView, 5, 200);
        RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) mIndexView.getLayoutParams();
        params.addRule(RelativeLayout.ALIGN_BOTTOM, mScaleView.getId());
    ...
    private void playNoHeadsetAnimation() {
        Log.e(TAG, "playNoHeadsetAnimation");
        if (null == mService) {
    ...
    private void changeToMainLayout() {
        ...
        mNoEarPhoneTxt.setVisibility(View.GONE);
        // change to main layout
        mMainLayout.setVisibility(View.VISIBLE);
        mBtnPlayContainer.setVisibility(View.VISIBLE);

        mButtonAddToFavorite.setEnabled(true);
        mButtonPrevStation.setEnabled(true);
        mButtonNextStation.setEnabled(true);
    ...
    private void changeToNoHeadsetLayout() {
        ...
        mButtonAddToFavorite.setEnabled(false);
        mButtonPrevStation.setEnabled(false);
        mButtonNextStation.setEnabled(false);
        mButtonPlay.setEnabled(false);

        mNoHeadsetLayout.setVisibility(View.VISIBLE);
        mNoEarPhoneTxt.setVisibility(View.VISIBLE);
