...
public class FmUtils {
     ...
     */
    public static int computeIncreaseStation(int station,int step) {
     ...
     */
    public static int computeDecreaseStation(int station,int step) {
        int result = station - step;
     ...
     * @return
     */
    public static  List<SoundRecorder> getSoundRecorders(String dir) {
        List<SoundRecorder> soundRecorderList=new ArrayList<>();
        File file=new File(dir);
        File records[]= file.listFiles();
        if(records!=null){
            for(int i=0;i<records.length;i++){
                if(records[i].getName().indexOf(".")>=0){
                    String substring = records[i].getName().substring(records[i].getName().indexOf("."));
                    if(substring.toLowerCase().equals(".3gpp")){
                        String recordName=records[i].getName().substring(0,records[i].getName().indexOf("."));
                        SoundRecorder soundRecorder=new SoundRecorder();
                        soundRecorder.setPath(dir + File.separator+records[i].getName());
                        soundRecorder.setFileName(recordName);
                        soundRecorder.setIsOpen(false);
                        soundRecorderList.add(soundRecorder);
                    }
                }
            }
        }

        return soundRecorderList;
    }

    private final int RENAME_DEFAULT=0;
    private final int RENAME_FAILING=-1;
    private final int RENAME_SUCCEED=1;
    /**
     *
     * @param dir
     * @param oldName
     * @param newName
     * @return 0:oldName==newName or oldFile not exist,default
     *         -1:newFile is exist,failing
     *         1:succeed
     */
    public static int renameFile(String dir, String oldName,String newName){
